    ...
    public async Task CharacterSelecteAreaController(int serverNum)
    {
        ...
        switch (serverNum)
        {
            case 0:
                ...
                await FirebaseManager.Instance.LoadCharacter(user.UserId, "server1", OnCharacterLoaded);
                break;
            case 1:
                characterSelectedArea[1].SetActive(true);
                await FirebaseManager.Instance.LoadCharacter(user.UserId, "server2", OnCharacterLoaded);
                break;
            case 2:
                characterSelectedArea[2].SetActive(true);
                await FirebaseManager.Instance.LoadCharacter(user.UserId, "server3", OnCharacterLoaded);
                break;
