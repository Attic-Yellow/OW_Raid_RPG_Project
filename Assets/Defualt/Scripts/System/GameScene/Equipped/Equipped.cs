...
public class Equipped : MonoBehaviour
{
    ...
    public List<Equipment> GetEquipmentList(EquipmentType gearType)
    {
        switch (gearType)
        {
            ...
            case EquipmentType.Hands: return hands;
    ...
    {
        List<Equipment> list = GetEquipmentList(equipment.equipment);
        ...
        {
            if (list[i] != null && list[i].equipment == EquipmentType.None)
            {
                list[i] = equipment;
                GameManager.Instance.uiManager.gameSceneUI.characterGearUI.equipmentType = equipment.equipment;
                onChangeGear?.Invoke();
                return true;
            }
        }
    ...
    public bool AssignEquipAtIndex(Equipment equipment, int oldIndex, int newIndex, SlotType oldSlotType, SlotType newSlotType)
    {
        List<Equipment> list = GetEquipmentList(equipment.equipment);
        ...
        {
            if (list[newIndex].equipment != EquipmentType.None) // print("현재 장비 > 장비함 스왑");
            {
                CurrentEquipped.Instance.IsEquipped(list[newIndex], oldIndex, newIndex, newSlotType, newSlotType); 
                list[newIndex] = equipment;
            }
            else
            {
                list[newIndex] = equipment; // print("현재 장비 > 장비함 넣기");
            }
        }
        else if (newSlotType == SlotType.CurrentEquip)  // print("장비함 > 현재 장비 스왑");
        {
            CurrentEquipped.Instance.IsEquipped(equipment, oldIndex, oldIndex, oldSlotType, oldSlotType);
        }
        else
        {
            if (list[newIndex].equipment != EquipmentType.None)  // print("장비함 > 장비함 스왑");
            {
               
                list[oldIndex] = list[newIndex];
                list[newIndex] = equipment;
            }
            else // print("장비함 장비 추가");
            {
                list[newIndex] = equipment;
            }
        }
    ...
    #region 슬롯 초기화 메서드
    public void RemoveEquip(Equipment equipment, int index)
    {
        List<Equipment> list = GetEquipmentList(equipment.equipment);

        if (list[index] != null)
        {
            list[index] = new Equipment(); 
            GameManager.Instance.uiManager.gameSceneUI.characterGearUI.equipmentType = equipment.equipment;
            onChangeGear?.Invoke();
        }
    }
